    ...
    <div class="wrapper">
            ...
            <form id="add-bill-form" enctype="multipart/form-data">
                <div class="modal-dialog modal-lg" role='document'>
                    ...
                    while ($row = $res->fetch_assoc()) {
                      $sql = "SELECT * FROM tenants WHERE room_id = " . $row['id'] . " WHERE `status` = 0 AND account_status = 0";
                    ...
                    foreach ($rooms as $room) {?>
                            ...
                            <div class="row mb-2">
                                <div class="col-sm-12"
                                    ...
                                    <button type='button' class="btn btn-sm btn-secondary" id="add-charge">New
                                        &#43;</button>
                                </div>
                            </div>
      ...
      `;
            ...
            $('select[name=room_name]').change(function(e) {
                ...
                val === '' ? billToSelect.attr('disabled', 'disabled') : billToSelect.removeAttr(
                    'disabled');
            ...
            function renderBillToOptions(elem, val) {
                ...
                $.get("./../functions/get_room_tenants.php?id=" + val, function(res) {
                    ...
                    const options = data.map(d =>
                        `<option value="${d.id}">${d.first_name} ${d.last_name}</option>`).join(
                        ' ');
